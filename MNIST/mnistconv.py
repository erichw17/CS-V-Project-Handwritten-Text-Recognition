...
def step (c, cb, w, b, x, y, rate):
    size = x.shape[0]
    conv_scores = conv(c, cb, x)
    relu_scores = relu(conv_scores)
    final_scores = np.dot(conv_scores, w) + b

    correct_vals = final_scores[np.arange(size), y]
    final_grad = (final_scores.transpose() - correct_vals + 1).transpose()
    final_grad[np.arange(size), y] = 0
    final_grad[final_grad < 0] = 0
    loss = np.sum(final_grad)/size

    final_grad[final_grad > 0] = 1
    correct_grad = np.sum(final_grad, axis = 1)
    final_grad[np.arange(size), y] = -correct_grad
    ...
    while not done:
        for event in pygame.event.get():  
            if event.type == pygame.QUIT:
                ...
                r = np.random.randint(size-train_size)
                redraw(x[r], predict(w1, b1, w2, b2, x[r]), screen)
    pygame.quit()
...
layer = 1000
epochs = 18
print("1 conv layer 3x3x5 relu activation 1 fully connected layer")
